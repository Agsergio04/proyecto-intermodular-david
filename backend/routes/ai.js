 ...
 * @function
 * @memberof module:routes/ai
 * @param {express.Request<any, any, NextQuestionRequest>} req Petición con historial y contexto.
 * @param {express.Response} res Respuesta con la siguiente pregunta generada.
 * @returns {Promise<void>}
 * @access Private (JWT requerido)
 * @todo Implementar validaciones detalladas y manejo de errores de IA.
 */
router.post('/next-question', authMiddleware, async (req, res) => {
  // Implementación pendiente: validar entrada, construir prompt y devolver JSON estructurado.
  // Mantener el middleware de autenticación y devolver 503 si `ai` no está configurado.
  if (!ai) {
    return res.status(503).json({ message: 'AI service not available. GEMINI_API_KEY not configured.' });
  }
  try {
 ...
 * @function
 * @memberof module:routes/ai
 * @param {express.Request<any, any, EvaluateResponseRequest>} req Petición con `question`, `response` y `profession`.
 * @param {express.Response} res Respuesta con la evaluación estructurada (puntuación, comentarios).
 * @returns {Promise<void>}
 * @access Private (JWT requerido)
 * @todo Implementar la evaluación real usando el cliente `ai` y normalizar el JSON de salida.
 */
